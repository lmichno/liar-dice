...
const lobbyIdG = urlParamsG.get('lobbyId');
if (!lobbyIdG) {
...
let gameState = null;
let playerNameG = sessionStorage.getItem('playerName');
wsG.onopen = () => {
...
function updateGame(data) {
    ...
    data.players.forEach(player => {
        ...
        if (player.name === data.currentTurn)
...
function updateActions(currentBet) {
    ...
    const maxDiceCount = (gameState === null || gameState === void 0 ? void 0 : gameState.players.reduce((sum, player) => sum + player.dice.length, 0)) || 0;
